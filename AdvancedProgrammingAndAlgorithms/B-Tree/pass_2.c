...
void free_and_flush(void *b_tree, void *tree_node)
{
	...
	//look through the nodes up to the root
	while(tn->parent != NULL){
		...
		/* if flush is not set then add the node to the free list */
			...
			//set ptr to the rest of the free_list then set the free list to the node
			//printf("setting this on the free list\n");
			//printf("tn->keys: %s\n\n", tn->keys[0]);	
			printf("dont flush: %d\n", tn->lba);
			tn->ptr = bt->free_list;
			bt->free_list = tn;
		}
		//free_and_flush(bt, tn->parent);
		tn = tmp;	
	}

	//flush the root node if needed
	if(tn->flush == 1){
		printf("flush root node: %d\n", tn->lba);
		/*
		printf("tn->nkeys: %d\n", tn->nkeys);
		...
		*/	
		...
		printf("root node: %d\n", tn->lba);
		printf("dont flush add to free list\n");
		//printf("setting this on the free list\n");
		//printf("tn->keys: %s\n\n", tn->keys[0]);	
		tn->ptr = bt->free_list;
		bt->free_list = tn;
	...
	int k = 0;
	Tree_Node *list_size;
	list_size = bt->free_list;
	while(list_size != NULL){
		list_size = list_size->ptr;
		k++;
	}
	printf("list_size: %d\n", k);
...
int split_node(void *b_tree, void *t_node, unsigned long first_free_block)
{
	...
	if(new_root == 1){
		if(bt->free_list != NULL){
			...
			parent->keys = malloc((bt->max_key+1) * bt->key_size);
...
void *b_tree_create(char *filename, long size, int key_size)
{
	...
	//printf("node keys size: %d\n", (sizeof(unsigned char) * (bt->max_key+1) * bt->key_size));
	//printf("node lbas size: %d\n", (sizeof(unsigned int) * (bt->max_key+2)));
	...
	for(i=0; i < (bt->max_key+1); i++){
		...
		//root->keys[i] = (unsigned char*)&root->bytes+2+(bt->key_size * i);
	}
	//printf("here\n");	

	//memcpy(root->bytes, root->keys, (sizeof(unsigned char*) * (bt->max_key+1) * bt->key_size));
	//((unsigned char*)root->bytes+2) = root->keys;
